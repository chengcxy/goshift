...
func (m MysqlPlugin) ExecuteTask(wid int, task *Task, finishedChan chan int, tm *meta.TaskMeta, writer Plugin) *Result {
	...
	fmt.Println("ExecuteTask start is ", start, " end is ", end)
	...
	if len(values) > 0 {
		...
		if err != nil {
			fmt.Println("insert error", insertSql, err)
		}
		num, _ := r.RowsAffected()
		syncNum += num
		values = values[:0]
...
func (m MysqlPlugin) GetTotalSplits(start, end int64, batch int) (Splits []*TaskParams) {
	for start < end {
		_next := start + int64(batch)
		if _next >= end {
			_next = end
		}
		Splits = append(Splits, &TaskParams{start: start, end: _next})
		start = _next
	}
	return
}
...
func (m MysqlPlugin) Read(ctx context.Context, writer Plugin, tm *meta.TaskMeta) error {
	...
	if err != nil {
		fmt.Println("get min max error", err)
		return err
	}
	minId = minId - 1
	fmt.Println("MinId:", minId)
	fmt.Println("MaxId: ", maxId)
	Splits := m.GetTotalSplits(minId, maxId, tm.ReadBatch)
	...
	go func() {
		...
		for range finishedChan {
			finished += 1
			fmt.Printf("[finished process is %d/%d,unfinished is %d/%d] \n", finished, totalTask, totalTask-finished, totalTask)
		}
	}()
	...
	go func() {
		for wid := 0; wid < tm.WorkerNum; wid++ {
			w := <-dones
			fmt.Printf("workerid %d executed:%d \n ", w.wid, w.executed)
		}
		close(finishedChan)
		close(resultChan)
	}()
