...
func (tp TrinoPlugin) Read(ctx context.Context, writer Plugin, tm *meta.TaskMeta) *TaskResult {
	...
	go func() {
		for wid := 0; wid < tm.WorkerNum; wid++ {
			w := <-dones
			logger.Infof("workerid %d executed %d tasks", w.wid, w.executed)
		}
		close(finishedChan)
		close(resultChan)
	}()
	...
	for r := range resultChan {
		...
		logger.Infof("taskIndex:%d (start:%d:end:%d),wid:%d,syncNum:%d,status:%d", r.taskParam.index, r.taskParam.start, r.taskParam.end, r.wid, r.syncNum, r.status)
